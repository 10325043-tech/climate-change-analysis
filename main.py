...
st.markdown("""
    ...
    .radar-box { background: rgba(0, 0, 0, 0.6); border: 1px solid #38bdf8; padding: 10px; }
    </style>
...
if st.session_state.state == "HOME":
    ...
    data = [("OCEANIA", 34, 38, "SECURE", "#00ff9d"), ("ASIA", 34, 32, "CRITICAL", "#ff4d4d"), ("EUROPE", 22, 30, "SECURE", "#00ff9d"), ("AFRICA", 28, 35, "SECURE", "#00ff9d"), ("NORTH AMERICA", 34.2, 36, "SECURE", "#00ff9d"), ("SOUTH AMERICA", 31.8, 32, "SECURE", "#00ff9d")]
    cols = st.columns(3)
    ...
    with l_col:
        ...
        sel_nations = st.multiselect("NATIONS", countries, default=[countries[0]])
        years = st.slider("YEAR RANGE", 2000, 2012, (2000, 2012))
        st.markdown('</div>', unsafe_allow_html=True)
    
    with mid_col:
        sub = df[(df['Country'].isin(sel_nations)) & (df['Year'] >= years[0]) & (df['Year'] <= years[1])]
    ...
    with r_col:
        ...
        for n in sel_nations:
            radar.add_trace(go.Scatterpolar(r=[5, 3, 8], theta=['Temp', 'Volatility', 'Risk'], fill='toself', name=n))
        radar.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, 10])), showlegend=False, paper_bgcolor="rgba(0,0,0,0)")
