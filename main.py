...
if st.session_state.state == "HOME":
    ...
    df_filtered = df # Add filter logic based on your continent mapping if needed
    years = sorted(df_filtered['Year'].unique())
    y = st.select_slider("SELECT YEAR", options=years)
    
    c1, c2 = st.columns([1, 1])
    with c1:
        fig_map = px.choropleth(df_filtered[df_filtered['Year'] == y], locations="Country", locationmode="country names", color="AverageTemperature", color_continuous_scale="RdYlBu_r")
    ...
    with c2:
        sel = st.multiselect("SELECT COUNTRIES", sorted(df_filtered['Country'].unique()))
        if sel:
            fig_line = px.line(df_filtered[df_filtered['Country'].isin(sel)], x="Year", y="AverageTemperature", color="Country", template="plotly_dark")
